...
subprocess.run([str(yt_dlp), '-U'], check=True)
...
class PartialClipDownloaderApp:
    ...
    def download_video(self):
        ...
        from_browser = [
            str(yt_dlp),
            "--downloader", "ffmpeg", "-N", '6',
            '-f', 'bestvideo+bestaudio',
            "-o", f"%(title)s-{time_str}.%(ext)s",
            "--download-sections", f"*{start_time}-{end_time}",
            "--cookies-from-browser", r"firefox",
            "--add-header",
        ...
        command = [
            ...
            "-o", f"%(title)s-{time_str}.%(ext)s",
            "--download-sections", f"*{start_time}-{end_time}",
            "--cookies", r"C:\Users\dower\Desktop\youtube_cookies.txt",
            '--merge-output-format', 'mp4',
            f'--verbose',
            "--add-header",
            "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3",
        ...
        try:
            ...
            subprocess.run(from_browser, check=True)
